...
function Modal() {
    ...
    useEffect(() => {
        ...
        async function fetchMovie() {
            const data = await fetch(
                ...
                }/${movie?.id}?api_key=${process.env.NEXT_PUBLIC_API_KEY}&language=en-US&append_to_response=videos`
            ).then((res) => res.json())
            if (data?.videos) {
    ...
    return (
        <MuiModal open={showModal} onClose={handleClose} className="fixed !top-7 left-0 right-0 z-50 mx-auto w-full max-w-5xl overflow-hidden overflow-y-scroll rounded-md ">
            <>
                <button onClick={handleClose} className="modalButton absolute right-2 md:right-5 top-5 !z-50 h-9 w-9 border-none bg-[#181818]">
                ...
                <div className='relative pt-[56.25%]'>
                    {trailer ? <ReactPlayer
                        ...
                        playing
                        muted={muted}
                    />: <div className='absolute top-0 left-0 w-full h-full bg-black flex justify-center items-center font-semibold text-xl text-gray-400'>
                        : ( No video available
                        </div>}
                        ...
                        <div className='flex flex-col gap-x-10 gap-y-4 md:flex-row'>
                            ...
                            <div className='flex flex-col space-y-3 text-sm'>
                                <div>
                                    ...
                                    {genre.map((genres) => genres.name).join(', ')}
                                </div>
                                <div>
